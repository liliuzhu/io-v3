...
var tetris={//游戏主程序对象
	...
	keys:function(num)
	{
		switch(num)
		{
			case 37:this.state==this.RUNING&&this.moveLeft();break;//方向左键左移
			case 39:this.state==this.RUNING&&this.moveRight();break;//方向右键右移
			case 40:this.state==this.RUNING&&this.moveDown();break;//方向下键下移
			case 38:this.state==this.RUNING&&this.rotateR();break;//方向手上键右旋
			case 90:this.state==this.RUNING&&this.rotateL();break;//z键左旋
			case 83:this.state==this.GAMEOVER&&this.start();break;//s键结束后开始
			case 80:this.state==this.RUNING&&this.pause();break;//p暂停
			case 67:this.state==this.PAUSE&&this.myContinue();break;//c键继续
			case 81:this.gameOver();this.paint();break;//q键gameOver
			case 32:(this.state==this.RUNING||this.state==this.stop)&&this.hard();break;//空格键停止下移
		}
	},
	hard:function()
	{
	...
	upDateInter:function()
	{
		...
		if(this.level!=this.upDate)
		{
			...
			$("pg").appendChild(img);
			this.hard();
			document.body.querySelector("img[src='img/升级.png']").addEventListener("touchend",this.keys.bind(this,32));
		}
	},
	paintState:function()
	{
		if(this.state!=this.RUNING&&this.state!=this.stop)
		{
			...
			$("pg").appendChild(img);
			this.state!=this.PAUSE&&document.body.querySelector("img[src='img/game-over.png']").addEventListener("touchend",this.keys.bind(this,83));
		}
	},
...
var direction={
	...
	ends:function()
	{
		...
		switch(this.dire)
		{
			case "左划": tetris.keys(37);break;
			case "上划": tetris.keys(38);break;
			case "右划": tetris.keys(39);break;
			case "下划": tetris.keys(40);break;
			default:break;
		}
		this.dire=null;
		this.movex=null;
